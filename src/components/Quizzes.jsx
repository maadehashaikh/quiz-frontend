...
//               <p>03-01-2023</p>
//               <p>
//                 <i className="fa-solid fa-clock mr-3"></i>12:30 AM - 01:40 PM
//               </p>
//             </div>
      ...
      try {
        ...
        if (response.status === 401 && refreshToken) {
          ...
          if (refreshResponse.ok) {
            ...
            response = await fetch("http://127.0.0.1:8000/api/mcqs/", {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
  ...
  return (
    <div className="flex h-screen bg-black">
      ...
      <div className="w-[85%] flex flex-col ">
        ...
        <div className="h-[74%] rounded-lg bg-black flex items-start justify-between">
          <div className="w-2/3 bg-white p-3 mt-2 ml-2 rounded-sm">
            {quizzes.length > 0 ? (
              quizzes.map((quiz) => (
                <div key={quiz.id} className="border-b border-gray-300 pb-4 mb-4">
                  <div className="text-black text-xl">{quiz.title}</div>
                  <h3 className="text-gray-400 pt-2">Course: {quiz.course_name}</h3>
                  <h3 className="text-gray-400 py-2">Subject: {quiz.subject}</h3>
                  <div className="flex py-3 items-center justify-evenly">
                  ...
                  <p className="text-gray-400">Total Questions: {quiz.questions.length}</p>
                  <div className="flex items-center justify-between mt-2">
                    <p>Passing percentage</p>
                    <p className="text-blue-300">{quiz.passing_percentage}%</p>
                  </div>
                  <button className="bg-red-600 w-full py-2 mt-2 text-white"
